/**
 * User-facing slash commands. Pure transforms over the raw prompt string.
 *
 * Lifted verbatim from the old desktop renderer.
 */
...
export const SLASH_COMMANDS: ReadonlyArray<SlashCommand> = [
  ...
  },
  {
    ...
    transform: (body) =>
      `Translate the following into English. Preserve formatting.\n\n${body}`.trim(),
  },
  {
    name: 'rewrite',
    description: 'Rewrite the following clearly and concisely',
    transform: (body) =>
      `Rewrite for clarity and concision. Preserve meaning.\n\n${body}`.trim(),
  },
  {
    name: 'refine',
    description: 'Polish for tone, grammar, and flow',
    transform: (body) =>
      `Polish the following text for tone, grammar, and flow. Return only the result.\n\n${body}`.trim(),
  },
  {
    name: 'bullets',
    description: 'Convert the following into a bulleted list',
    transform: (body) =>
      `Convert the following into a tight bulleted list.\n\n${body}`.trim(),
  },
  {
    name: 'todos',
    description: 'Extract actionable todos',
    transform: (body) =>
      `Extract actionable todos from the following as a checklist.\n\n${body}`.trim(),
  },
  {
    name: 'think',
    description: 'Reason carefully step-by-step before answering',
    transform: (body) =>
      `Think carefully and reason step-by-step before giving a final answer.\n\n${body}`.trim(),
  },
]
