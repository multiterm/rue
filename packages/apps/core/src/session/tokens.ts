/**
 * Rough token estimation for context-budget checks.
 *
 * ~4 characters per token is the well-known heuristic for English text — it
 * is close enough for a budget gate and avoids bundling a full BPE tokenizer
 * (which would differ per model anyway).
 */
